...
async function testProvider(rpcUrl, provider) {
  ...
  const averageBalance =
    balanceTimes.reduce((a, b) => a + b, 0) / balanceTimes.length;
  results[rpcUrl] = {
    estimateGas: average,
    "Ping speed": pingResult.avg,
    getBlock: averageBlock,
    getBalance: averageBalance,
  };
}
...
async function testWsProvider(rpcUrl) {
  const customWebSocketProvider = new ethers.providers.WebSocketProvider(
    rpcUrl,
    {
      chainId: network.chainId,
      name: network.name,
...
if (type === "http") {
  Promise.all(rpcUrls.map(testHttpProvider))
    .then(() => {
...
if (type === "wss") {
  Promise.all(rpcUrls.map(testWsProvider))
    .then(() => {
      console.table(results);
      process.exit(0);
    })
